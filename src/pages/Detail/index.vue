        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 之前路由跳转，就是从a到b，但注意加入购物车，进行路由跳转之前，发请求把购买的产品信息通过请求的形式通知服务器进行存储 -->
...
export default {
  ...
  mounted() {
    // 派发action，获取商品详情数据，再从中获取需要的'categoryView', 'skuInfo', 'spuSaleAttrList'
    this.$store.dispatch('getGoodsInfo', this.$route.params.skuId)
  },
  computed: {
    // 层级较多，仓库中使用getters进行简化再映射，用数组格式，mapState用对象形式
  ...
  methods: {
    // 产品的售卖属性切换高亮效果
    changeActive(saleAttrValue, attrList) {
      //遍历全部售卖属性值的isChecked = 0，就没有高亮了
    ...
    // 加入购物车的回调
    async addShopCart() {
      // 1.发请求--通知服务器要加入购物车的产品信息
      // 当前这里是派发一个action,调用了addOrUpdateShopCart，如果执行会返回Promise
      // 问题：如何判断加入购物车成功还是失败？需要等待执行结果，加await
      try {
        // 2.服务器存储成功---进行路由跳转
        await this.$store.dispatch('addOrUpdateShopCart', {
          skuId: this.$route.params.skuId, //可以到开发者工具里看
          skuNum: this.skuNum,
        })
        // 进行路由跳转,带着产品信息过去
        // 一些简单的数据skuNum，可以通过query传递
        // 一些复杂的skuInfo,通过会话存储[不持久化,会话结束数据消失],一般存储的是字符串
